...
class CsgoViewModel(val database: CryptonicDatabaseDao, application: Application) : AndroidViewModel(application) {
    ...
    private fun getMatches() {
        coroutineScope.launch {
            try {
                ...
                _matches.value = listResult!!.filter { Match -> Match.videogame!!.id === 3 }
            } catch (e: Exception) {
